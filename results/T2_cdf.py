...
def create_data(list_values):
    values = list(map(float, ['%.1f' % elem for elem in list(np.linspace(0.1,7.0,100))]))
    count =  [0]*100
    #print(values)
...
ax.legend(handles=[pb, pe, pf, pg], fontsize=14, loc='lower right')
...
plt.savefig("T2_cdf.pdf", bbox_inches='tight')
plt.show()