...
def DRC_structure_T2():
    ...
    DRCs = {7: DRC7, 8: DRC8, 9: DRC9, 10: DRC10,
            17:DRC7_B, 18: DRC8_B, 19: DRC9_B, 20: DRC10_B,
            28: DRC8_C}
...
def run_stage_0():
    ...
    i = [(p, d, b) for p in paths for d in DRCs for b in rus if (paths[p].seq[2] == rus[b].CR and paths[p].seq[0] == 0 and (paths[p].seq[1] == 0 or paths[p].seq[1] == fixed_cu)  and (paths[p].mec_loc == fixed_cu or paths[p].mec_loc == paths[p].seq[2])) and
    ...
    for b in rus:
        if rus[b].CR in mec:
            mdl.add_constraint(mdl.sum(mdl.max((mdl.x[it]*(paths[it[0]].delay_p_mec + (paths[it[0]].delay_p2*mdl.min(1, paths[it[0]].seq[0])) + (paths[it[0]].delay_p3*mdl.min(1, paths[it[0]].seq[1])) + ((((MEC_proc)/(crs[paths[it[0]].mec_loc].cpu))*MEC_load)*0.25 + pow(((MEC_proc)/(crs[paths[it[0]].mec_loc].cpu))*MEC_load, 2)*0.25))for it in i if rus[it[2]].CR in mec and b==it[2])))  >= d_function_limit, 'bottom limit of aplication(delay)')
    ...
    # Restrições para assegurar que DRCs agregadas não escolham caminhos onde o serv. mec esteja desagregado de seq
    mdl.add_constraint(mdl.sum(mdl.x[it] for it in i if (it[1] in [11,12,14,15,16,17,18,19,20]) and 
        (paths[it[0]].mec_loc != paths[it[0]].seq[0] and paths[it[0]].mec_loc != paths[it[0]].seq[1] and paths[it[0]].mec_loc != paths[it[0]].seq[2])) == 0, 'path')

    # Restrições para assegurar que DRCs desagregadas não escolham caminhos onde o serv. mec esteja agregado a seq
    mdl.add_constraint(mdl.sum(mdl.x[it] for it in i if (it[1] in [21,22,24,25,26,27,28,29,30]) and 
        (paths[it[0]].mec_loc == paths[it[0]].seq[0] or paths[it[0]].mec_loc == paths[it[0]].seq[1] or paths[it[0]].mec_loc == paths[it[0]].seq[2])) == 0, 'path')

    # Restrições de desativação de DRCs principais para RUs com demanda MEC
    mdl.add_constraint(mdl.sum(mdl.x[it] for it in i if it[1] in [1,2,4,5,6,7,8,9,10] and rus[it[2]].CR in mec) == 0, 'path')
...
def run_stage_1(FO_Stage_0):
    ...
    for b in rus:
        if rus[b].CR in mec:
            mdl.add_constraint(mdl.sum(mdl.max((mdl.x[it]*(paths[it[0]].delay_p_mec + (paths[it[0]].delay_p2*mdl.min(1, paths[it[0]].seq[0])) + (paths[it[0]].delay_p3*mdl.min(1, paths[it[0]].seq[1])) + ((((MEC_proc)/(crs[paths[it[0]].mec_loc].cpu))*MEC_load)*0.25 + pow(((MEC_proc)/(crs[paths[it[0]].mec_loc].cpu))*MEC_load, 2)*0.25))for it in i if rus[it[2]].CR in mec and b==it[2]))) <= worsening_limit, 'worsening_limit')
    ...
    data_generator(mdl, i, FO_Stage_0, worsening_limit, MEC_proc, MEC_load, rus, mec, paths, DRCs, crs, conj_Fs, data_file)
...
if __name__ == '__main__':
    ...
    if args.worsening_limit:
        ...
        worsening_limit = float(args.worsening_limit)
        print(worsening_limit)

    start_all = time.time()
    FO_Stage_0 = run_stage_0()
    FO_Stage_1 = run_stage_1(FO_Stage_0)
    end_all = time.time()
